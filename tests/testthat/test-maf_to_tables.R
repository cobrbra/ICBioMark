...
test_that("A simple call to get_table_from_maf() has the right structure and value", {
  output <- get_table_from_maf(maf = example_maf_data$maf, sample_list = paste0("SAMPLE_", 1:20))

  expect_is(output, "list")
  expect_equal(length(names(output)), 5)
  expect_equal(names(output), c("matrix", "sample_list", "gene_list", "mut_types_list", "colnames"))
...
test_that("A simple call to get_tables() has the right structure and value", {
  output <- get_tables(maf = example_maf_data$maf, sample_list = paste0("SAMPLE_", 1:20))
  ...
  expect_equal(names(train_data), c("matrix", "sample_list", "gene_list", "mut_types_list", "colnames"))
